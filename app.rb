...
class NostrEvent < ActiveRecord::Base # rubocop:disable Style/Documentation
...
def fetch_events(since) # rubocop:disable Metrics/AbcSize,Metrics/MethodLength
  nostr_events = []
  exitstatus = 0
  LOGGER.debug("Timeout seconds: #{NOSDUMP_TIMEOUT_SECONDS}")
  LOGGER.debug('Before Open3.popen3')
  Open3.popen3('timeout', NOSDUMP_TIMEOUT_SECONDS.to_s, 'nosdump', '--since', since, '--authors', AUTHORS.join(','), *RELAYS) do |stdin, stdout, _, wait_thr| # rubocop:disable Layout/LineLength
